...
public class NamesAdapter extends RecyclerView.Adapter<NamesAdapter.NameViewHolder> {
    ...
    public void onBindViewHolder(@NonNull NameViewHolder nameViewHolder, int position) {
        ...
        int color;
        if(position % 2 == 0){
            color = ContextCompat.getColor(nameViewHolder.itemView.getContext(),R.color.colorAccent);
        }else{
            color = ContextCompat.getColor(nameViewHolder.itemView.getContext(),R.color.colorPrimary);
        }

        ImageViewCompat.setImageTintList(nameViewHolder.ivIcon, ColorStateList.valueOf
                (color));
    }
